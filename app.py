...
def fetch_narou_data(genre_code, order_code, tensei_code):
    ...
    payload = {
        ...
        "lim": 400,                 
        "order": order_code,
        # 【修正】末尾のパラメータを「ap」から正しいなろう仕様「a」へ修正
        "of": "t-w-n-s-k-g-gp-l-f-a" 
    }
...
if novels and not filtered_novels:
    ...
    for idx, novel in enumerate(filtered_novels[:5]):
        with st.container():
            ...
            st.caption(f"作者: {novel['writer']} | ジャンル: {genre_name} | 総文字数: {novel['length']:,}文字 | ブックマーク: {novel['fav_count_display']:,} | 評価点: {novel['all_point_display']:,}pt | 総合ポイント: {novel['global_point']:,}")
