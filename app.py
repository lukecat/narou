...
def fetch_narou_data(genre_code, order_code, tensei_code):
    ...
    payload = {
        ...
        "lim": 400,                 
        "order": order_code,
        # 【重要】出力パラメータに「its（異世界転生フラグ）」と「iti（異世界転移フラグ）」を追加
        "of": "t-w-n-s-k-g-gp-l-f-a-its-iti" 
    }
...
if novels and not filtered_novels:
    if not ignore_max_point:
        st.warning("⚠️ 候補作品が見つかりません。ポイントが非常に高い作品が多い、または「評価ポイントを受け取らない」作品が多かった可能性があります。「総合ポイント上限による除外を無効にする」チェックをONにするか、各フィルター条件を少し緩めてみてください。")
    else:
    ...
    for idx, novel in enumerate(filtered_novels[:5]):
        with st.container():
            ...
            st.caption(f"作者: {novel['writer']} | ジャンル: {genre_name}{tag_text} | 総文字数: {novel['length']:,}文字 | ブックマーク: {novel['fav_count_display']:,} | 評価点: {novel['all_point_display']:,}pt | 総合ポイント: {novel['global_point']:,}")
